...
/* Har bir kodeksdagi modda soni.
   ...
   1) `.hint({ lawCode: 1 })` — MongoDB indeksdan foydalanishga
      ...
      umuman o'qilmaydi;
   2) natija 60 soniyaga keshlanadi — modda soni faqat yuklash
      paytida o'zgaradi, ya'ni har so'rovda qayta hisoblash keraksiz. */
...
async function getArticleCounts() {
  const now = Date.now();
  if (countCache && now - countCacheAt < COUNT_CACHE_MS) return countCache;

  const counts = await LegalChunk.aggregate([
    { $group: { _id: "$lawCode", count: { $sum: 1 } } },
  ]).hint({ lawCode: 1 });

  countCache = Object.fromEntries(counts.map((c) => [c._id, c.count]));
  countCacheAt = now;
  return countCache;
}
...
router.get("/laws/:code", userGuard, async (req, res) => {
  try {
    ...
    const items = await LegalChunk.find(filter)
    ...
    /* Tartiblash: "10-modda" "9-modda"dan KEYIN kelishi kerak (matn
       bo'yicha saralanса "10" < "9" bo'lib qolardi). Qo'shimcha
