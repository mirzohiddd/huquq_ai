...
export default function Constitution() {
  ...
  /* Hero'dagi "bob" ko'rsatkichi.
     ⚠️ `flat.length` ISHLATILMAYDI: ba'zi bo'limlarning o'z bobi yo'q
     (masalan OLTINCHI BO'LIM) va ular uchun yuqorida SUN'IY bob
     yaratiladi — moddalar bobsiz qolmasligi kerak. Uni ham sanash
     haqiqiy bo'lmagan raqam berardi, shuning uchun faqat nomi BOR
     boblar hisoblanadi (`unnamed` bayrog'i shu uchun qo'yilgan). */
  const chapterCount = useMemo(
    () => tree.reduce((n, sec) => n + sec.chapters.filter((c) => !c.unnamed).length, 0),
    [tree],
  );
  ...
  return (
    <>
      ...
      {/* ⚠️ Hero FON RASMI bilan ishlaydi (`public/const-hero.webp`).
          Shu sabab bu yerda CSS bayroq lentalari, yon illyustratsiyalar,
          tarozi ikonasi va oltin bezak chizig'i YO'Q — ularning
          hammasi rasmning O'ZIDA chizilgan. Qo'shilsa ustma-ust
          tushib, ikki marta ko'rinardi. */}
      <div className={`${s.heroOuter} animate-fade-up`}>
        <div className={s.hero}>
          <div className={s.heroInner}>
            ...
            <span className={s.metaChip}>
              <Layers size={15} />
              <strong>{chapterCount}</strong> {t.const_chapters_count}
            </span>
            {data?.law?.url && (
              <a
