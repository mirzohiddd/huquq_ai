  ...
  return (
    <>
      ...
      <div className={`${s.heroOuter} animate-fade-up`}>
        {/* ── Hero ──
            ⚠️ Banner rasmi (`public/library-hero.jpg`) OLIB TASHLANDI
            (foydalanuvchi so'rovi, 2026-08-17): u 1280×640 nisbatda
            edi va `--hero-h` ga bo'ysunmagani uchun qolgan uchta
            hero'dan ancha baland chiqardi. Endi bu sahifa ham
            Testlar/Darslar/Konstitutsiya bilan bir xil andazadan
            foydalanadi: gradient fon + `min-height: var(--hero-h)`.

            Rasm ustiga foizli joylashuv bilan qo'yilgan matn ham
            kerak bo'lmay qoldi — matn endi oddiy oqimda, ya'ni
            har qanday ekranda o'z-o'zidan to'g'ri joylashadi. */}
        <header className={s.hero}>
          <div className={s.heroBody}>
            ...
            <div className={s.perks}>
              {[
                { Icon: ShieldCheck, label: t.library_perk1 },
                { Icon: RefreshCw, label: t.library_perk2 },
                { Icon: Search, label: t.library_perk3 },
                { Icon: ScrollText, label: t.library_perk4 },
              ].map(({ Icon, label }) => (
                <div key={label} className={s.perk}>
                  <span className={s.perkIcon}>
                    <Icon size={16} />
                  </span>
                  <span className={s.perkLabel}>{label}</span>
                </div>
              ))}
            </div>
          </div>
