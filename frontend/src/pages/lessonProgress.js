...
/* ⚠️ Kalitlar HISOBGA bog'langan (`utils/accountScope.js`).
   Avval ular umumiy edi va bitta qurilmadagi barcha hisoblar bitta
   progressni baham ko'rardi — A hisobida tugatilgan dars B hisobida
   ham "tugallandi" bo'lib ko'rinardi. */
const STATE_BASE = "lessonState";
const XP_BASE = "lessonXp";

const stateKey = () => scopedKey(STATE_BASE);
const xpKey = () => scopedKey(XP_BASE);

// Hisoblar aralashgan davrdan qolgan umumiy kalitlarni bir marta tozalaymiz
purgeSharedKeys([STATE_BASE, XP_BASE]);
...
export async function pullProgress() {
  try {
    ...
    write(stateKey(), merged);
    // XP — serverdagi qiymat asosiy (u yerda XP qoidalari qo'llanadi)
    if (typeof data.xp === "number") write(xpKey(), data.xp);
...
export async function completeLessonRemote(lessonId) {
  ...
  try {
    ...
    if (typeof data?.totalXp === "number") write(xpKey(), data.totalXp);
    return data;
  } catch {
