...
const { clearCache } = require("../utils/queryCache");
...
async function ingestAllLaws(onlyCode, { force = false } = {}) {
  ...
  /* Kutubxona keshini tozalash — aks holda yangilangan modda matni
     kesh muddati (10 daqiqa) o'tguncha eski holatda ko'rinardi.
     Faqat HAQIQIY o'zgarish bo'lganda: kodekslarning aksariyati
     odatda o'zgarmaydi va keshni bekorga tashlash Atlas M0 ni
     qayta yuklardi. */
  const changed = results.some(
    (r) => (r.added || 0) + (r.updated || 0) + (r.removed || 0) > 0,
  );
  if (changed) {
    const n = clearCache("law:");
    if (n) console.log(`🧹 Kutubxona keshi tozalandi (${n} yozuv)`);
  }
