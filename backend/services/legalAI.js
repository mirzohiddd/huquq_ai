...
/* ── MISTRAL va COHERE (2026-08-15 qo'shildi) ────────────────────
   ⚠️ NIMA UCHUN: ikkala kalit ham `.env` da ALLAQACHON bor edi, lekin
   chatga ULANMAGAN edi — Mistral faqat embedding uchun ishlatilardi,
   Cohere esa umuman hech qayerda. Shu sabab mavjud to'rt provayderning
   (Cloudflare, Groq, Gemini, Claude) kunlik kvotasi tugaganda AI
   BUTUNLAY javob bera olmay qolardi — holbuki yonida ikkita ishlaydigan
   xizmat bekor turardi.

   Tartib: Mistral — Groq'dan keyin (o'zbek tilida sifati yaxshi va
   bepul darajasi keng), Cohere — eng oxirida (o'zbekchasi sezilarli
   zaifroq, faqat "hech narsadan ko'ra yaxshiroq" zaxira). */
const MISTRAL_API_KEY = process.env.MISTRAL_API_KEY || null;
const MISTRAL_MODEL = process.env.MISTRAL_CHAT_MODEL || "mistral-small-latest";
const COHERE_API_KEY = process.env.COHERE_API_KEY || null;
/* Cohere modellari orasidan ATAYLAB `command-a-03-2025`: sinovda
   `command-r-08-2024` va `command-r7b` o'zbekcha javobni buzib
   ("Mehnat shartnoma, ishtirokchilar va...") qaytardi. */
const COHERE_MODEL = process.env.COHERE_CHAT_MODEL || "command-a-03-2025";

/* ── OPENAI (2026-08-15 qo'shildi) ────────────────────────────
   ⚠️ Kalit `.env` da ALLAQACHON bor edi, lekin hech qayerda
   ishlatilmasdi. `openai` npm paketi o'rnatilmagan — Mistral/Cohere
   kabi to'g'ridan-to'g'ri `fetch` bilan chaqiriladi, ya'ni yangi
   bog'liqlik qo'shilmaydi.

   ⚠️ HOZIRGI HOLAT: bu kalitning kvotasi TUGAGAN (429 "exceeded your
   current quota"). Kod to'g'ri ishlaydi va zanjir uni jim o'tkazib
   yuboradi — balans to'ldirilgan zahoti o'z-o'zidan ishlay boshlaydi.

   `gpt-4o-mini` ATAYLAB: arzon va RASMNI KO'RA OLADI — shu sabab u
   rasm zanjiriga ham qo'shildi (avval rasmni faqat Gemini va Claude
   ko'rardi, ikkalasi ham hozir ishlamayapti). */
...
5. AVVAL SAVOLNI ANIQ TUSHUN, keyin javob ber: foydalanuvchi AYNAN nimani so'raganini aniqla va shunga javob ber. Savol huquqiy maslahat emas, balki ma'lumot, tushuntirish yoki material haqida bo'lsa — uni maslahat qolipiga ZO'RLAB solma, oddiy va aniq javob ber. So'ralmagan narsani qo'shma.
6. ${CAPABILITY_RULE}
...
/* ── Mistral AI ───────────────────────────────────────────────
   OpenAI bilan mos API, shuning uchun `messages` xuddi Groq'dagidek
   uzatiladi — alohida format kerak emas. */
async function callMistral(messages, model) {
  if (!MISTRAL_API_KEY) throw new Error("MISTRAL_API_KEY sozlanmagan");
...
/* ── OpenAI ───────────────────────────────────────────────────
   `messages` OpenAI formatida keladi (system/user/assistant), ya'ni
   qo'shimcha o'girish kerak emas. Rasm berilgan bo'lsa oxirgi
   foydalanuvchi xabari matn + rasm juftligiga aylantiriladi. */
async function callOpenAI(messages, model, imgBase64, imgMime) {
  if (!OPENAI_API_KEY) throw new Error("OPENAI_API_KEY sozlanmagan");

  let payloadMessages = messages;
  if (imgBase64) {
    /* Oxirgi `user` xabariga rasm biriktiriladi — yangi xabar
       qo'shilsa suhbat tartibi buzilardi (user → user ketma-ket). */
      ...
      i === idx
        ? {
            ...
            content: [
              {
                type: "text",
                text: `Hujjat rasmini tahlil qil: nomlar, sanalar, summalar, imzolar, hujjat turi. Keyin javob ber: ${m.content}`,
              },
              {
                type: "image_url",
                image_url: {
                  url: `data:${imgMime || "image/jpeg"};base64,${imgBase64}`,
                },
              },
            ],
          }
...
/* ── Cohere ───────────────────────────────────────────────────
   ⚠️ Cohere v2 `system` rolini qo'llab-quvvatlaydi, lekin javob
   tuzilmasi BOSHQACHA: matn `message.content[]` massivida keladi
   (OpenAI'dagi `choices[0].message.content` EMAS). */
async function callCohere(messages, model) {
  if (!COHERE_API_KEY) throw new Error("COHERE_API_KEY sozlanmagan");
...
async function getLegalAdvice(
  ...
  // yuklab ber" so'rovi ichida "huquq" so'zi bo'lgani uchun u huquqiy
  // savol deb qabul qilinar va shoshilinch maslahat qolipiga tushardi.
  // Rasm yuborilgan bo'lsa tekshirilmaydi — u yerda foydalanuvchi
  // MAVJUD hujjatni tahlil qilishni so'raydi.
  if (!imageBase64 && isFileRequest(msg)) {
    return {
      answer: fileRequestReply(detectedLang, msg),
      category: "boshqa",
      provider: null,
    };
  }

  // 3. Huquqiy savol emas?
  ...
  /* ⚠️ OVOZDAN KELGAN SAVOL — mavzu filtri YUMSHOQ (2026-08-24).
     Nutq matnga o'girilganda so'zlar buziladi ("Ajrashsak bolam kim
     bilan qoladi?" → "Ajirashsak balam kim bilam qalad.") va kalit
     so'zga asoslangan filtr bunday matnda birorta ham naqshni topa
     olmaydi — foydalanuvchi ovozli sahifada javob o'rniga "Men faqat
     huquqiy savollarga javob beraman" degan rad javobini olardi.
     Ovoz sahifasi FAQAT huquqiy savol uchun mo'ljallangani va matn
     allaqachon `repairTranscript` dan o'tgani uchun bu yerda
     `inDialog` bilan bir xil yumshoq qoida qo'llanadi: faqat ANIQ
...
const PERSONAL_PATTERNS = [
  ...
  /мой муж|моя жена|у меня/i,
  /\bmy (husband|wife|boss|employer|landlord)\b|happened to me|i was (fired|beaten|cheated)/i,
];

/* 1-SHAXS OLMOSHI — O'ZI YETARLI EMAS.
   ⚠️ BUG FIX: avval "meni|menga|mening|bizga…" olmoshining O'ZI
   PERSONAL_PATTERNS ichida edi va "foydalanuvchi o'z holatini aytdi"
   degan ma'noni bildirardi. Lekin bu so'zlar AI'ga qaratilgan eng
   oddiy iltimosda ham bor: "menga tushuntirib ber", "menga ro'yxat
   ber", "menga dasturlarni yuklab ber". Natijada shunday HAR QANDAY
   xabar shoshilinch maslahat qolipiga ("Hozir nima qiling /
   Huquqlaringiz / Murojaat" + pichoqlab ketish namunasi) tushardi va
   foydalanuvchi so'ragan narsaga javob umuman berilmasdi.
   Endi olmosh faqat VOQEA belgisi bilan birga kelganda shaxsiy holat
   hisoblanadi ("meni ishdan bo'shatishdi", "menga pul bermayapti"). */
const SELF_PRONOUN =
  /\b(meni|menga|mendan|menda|mening|bizni|bizga|bizda|bizning)\b|меня|мне|нам|нас/i;

/* Voqea belgilari — ATAYLAB tor ro'yxat. Keng naqsh (masalan har
   qanday `\w+di`) oddiy savoldagi fe'lni ham ("taqsimlanadi") voqea
   deb qabul qilib, aynan shu xatoni qaytarardi. */
const EVENT_MARKERS = [
  /\b\w*(ishdi|ishmadi|shdi|dilar|mayapti|masdi|yotibdi)\b/,
  /ur(ib|di|apti|yapti|moqchi|gan)|kaltakla|zo.rla|tahdid|haqorat|so.k(di|yapti)/i,
  /bo.shat|hayda|quvib|ishdan\s*chiqar/i,
  /aldab|firib|o.g.irla|to.lamay|bermay|olmay|qaytarmay|undir(di|ishdi)/i,
  /buzdi|buzilgan|zarar\s*yetkaz|jarima\s*sol|sudga\s*ber(di|ishdi)/i,
  /уволил|избил|угрожа|не\s*плат|обманул|подал\s*в\s*суд/i,
];
...
function needsAdvice(text = "") {
  ...
  // Olmosh + voqea — yuqoridagi izohga qarang
  if (SELF_PRONOUN.test(low) && EVENT_MARKERS.some((p) => p.test(low)))
    return true;
...
async function askStructured({ system, user, isPro = false }) {
  ...
  // Mistral va Cohere — `getLegalAdvice` dagi bilan BIR XIL tartibda.
  // Aks holda dars amaliyoti chat ishlayotgan paytda ham yiqilardi.
  push("mistral", !!MISTRAL_API_KEY, () => callMistral(messages));
  push("gemini", !!geminiClient, () =>
    callGemini(system, user, [], null, null),
  );
  push("openai", !!OPENAI_API_KEY, () => callOpenAI(messages));
  push("cohere", !!COHERE_API_KEY, () => callCohere(messages));
...
/* `detectLanguage` EKSPORT QILINDI (2026-08-15) — "Eshitish" funksiyasi
   uchun. Ovoz javob YOZILGAN tilda chiqishi kerak, interfeys tilida
   emas: foydalanuvchi interfeysi o'zbekcha bo'lsa-yu savolni ruscha
   bersa, AI ruscha javob beradi va uni rus ovozi o'qishi kerak.
   Mantiq NUSXALANMADI — aks holda ikki joyda ikki xil bo'lib ketardi. */
module.exports = {
  getLegalAdvice,
  askStructured,
  providerStatus,
  detectLanguage,
  // `voiceTranscript.js` uchun: transkripsiya allaqachon huquqiy savol
  // sifatida tanilgan bo'lsa, uni AI bilan tuzatishga hojat yo'q.
  isLegalQuestion,
};
