...
function makeCollector() {
  ...
  return {
     ...
     * @param {{cachedOnly?: boolean}} [opts]
     *   `cachedOnly: true` — AI UMUMAN chaqirilmaydi: faqat keshdagi
     *   tarjima ishlatiladi, qolgani manba tilida qoladi va fonda
     *   tayyorlanadi. Bu rejim CHAQIRUVCHI KUTIB TURADIGAN joylar
     *   uchun (Telegram bot): AI kvotasi tugaganda `translateMany`
     *   provayder zanjirini aylanib chiqib, bitta dars uchun
     *   80 soniyagacha ketardi — foydalanuvchi shuncha vaqt hech
     *   qanday javob ko'rmasdi. Kutubxona/kutubxona sahifalaridagi
     *   qoida bilan bir xil: eng yomon holat "tarjima qilinmagan
     *   matn", "bo'sh ekran" emas.
     */
    async run(targetLang, opts = {}) {
      if (!texts.length) return;
      if (opts.cachedOnly) {
        const { result, missing } = await translateManyCached(
          texts,
          targetLang,
        );
        result.forEach((value, i) => setters[i](value));
        if (missing.length) warmTranslate(missing, targetLang);
        return;
      }
 ...
 */
async function translateLesson(lesson, targetLang, opts = {}) {
 ...
 */
async function translateLessonList(lessons, targetLang, opts = {}) {
 ...
 */
async function translateQuiz(quiz, targetLang, opts = {}) {
 ...
 */
async function translateQuizList(quizzes, targetLang, opts = {}) {
